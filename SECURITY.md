...
| **1Password CLI**               | `downloads.1password.com` dnf repo   | Not packaged by Fedora                                                                                           | Signing key is **fingerprint-verified** before `rpm --import`; the repo file is only written *after* a successful import, so a failed/rotated key costs `op`, not your package manager. On the atomic edition (Silverblue / Kinoite / bootc) `rpm --import` cannot lock the read-only rpmdb, so the *verified* key file is installed under `/etc/pki/rpm-gpg` and the repo's `gpgkey=` points at it (`file://`); `rpm-ostree` imports it when it layers from the repo |
| **starship / atuin / mise**     | Official install scripts             | Genuinely absent, or the packaged version trails a validated one                                                 | Downloaded to a temp file, sanity-checked, then run — never `curl \                                                                                                                                                                                                                                                                                                                                                                                                   |
