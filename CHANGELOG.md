...
- **The README opens with a rendered terminal hero** (dotgibson/dotfiles-core#948).
  ...
  re-render with `vhs assets/demo.tape` on a Fedora box after a prompt or tooling
  change, then `gifsicle -O3 --lossy=80 --colors 64` — the raw render is over Core's
  2 MiB ceiling, the optimised one is not.
